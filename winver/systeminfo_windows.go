...
// go env -w GOARCH=386 时可以调用成功,但是GOARCH=amd64时调用数据是不对的
package winver
...
func GetNativeSystemInfo() (*SystemInfo, error) {
	...
	mylog.Check3(procGetNativeSystemInfo.Call(uintptr(unsafe.Pointer(systemInformation))))
...
type DummyStructName struct {
	...
	*/
	...
	ProcessorArchitecture uint16 // 已安装操作系统的处理器体系结构。
	Reserved              uint16
}
...
typedef struct _SYSTEM_INFO {
    ...
    uint32_PTR dwActiveProcessorMask;
...
type SystemInfo struct {
	...
	PageSize                  uint32  // 虚拟内存页的大小
	MinimumApplicationAddress uintptr // 应用程序和动态链接库（DLL）可访问的最低内存地址
	MaximumApplicationAddress uintptr // 应用程序和动态链接库（DLL）可访问的最高内存地址
	ActiveProcessorMask       *uint32 // 表示配置到系统中的处理器集的掩码
	NumberOfProcessors        uint32  // 处理器数量
	ProcessorType             uint32  // 处理器类型
	AllocationGranularity     uint32  // 虚拟内存的起始地址
	ProcessorLevel            uint16  // 依赖于体系结构的处理器级别。它只能用于显示目的。要确定处理器的功能集，请使用IsProcessorFeaturePresent函数。
	ProcessorRevision         uint16  // 依赖于体系结构的处理器版本
}
...
func (d *DummyStructName) IsWin64() bool {
	return d.ProcessorArchitecture == uint16(PROCESSOR_ARCHITECTURE_AMD64) || d.ProcessorArchitecture != uint16(PROCESSOR_ARCHITECTURE_IA64)
}
