...
func processFileptrResponse(resp *http.Response) (int64, string) {
	fileptrBytes := mylog.Check2(ioutil.ReadAll(resp.Body))
...
func downloadSymbolFile(filepath string, url string) error {
	...
	if probeMethod == 2 {
		...
		_ = mylog.Check2(f.Read(byteData))
...
func main() {
	// https://githubfast.com/ajkhoury/pdbfetch
	// DecodeSysCall("C:\\Windows\\System32\\ntdll.dll")
	// DecodeSysCall("C:\\Windows\\System32\\win32u.dll")
	// DecodeSysCall("C:\\Windows\\System32\\ntoskrnl.exe") //iopxxx
	// DecodeSysCall("C:\\Windows\\System32\\win32k.sys")
	...
	if _ := mylog.Check2(os.Stat(pePath)); os.IsNotExist(err) {
	...
	for _, debugDir := range pefile.DebugDirectories {
		...
		if debugDir.InfoPdb70 != nil {
			...
			mylog.Check(downloadSymbolFile(downloadPath, downloadURL))
