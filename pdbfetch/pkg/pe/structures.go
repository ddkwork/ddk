...
func (s *SectionHeader) String() string {
	str := structString(s.fileOffset, "SECTION_HEADER", s.ImageSectionHeader) + flagString(s.flags)
	if len(s.Relocations) > 0 {
		str += fmt.Sprintf("Relocations:\n%-12s%-30s%s\n", "VA", "Type", "Symbol")
	}
	for _, reloc := range s.Relocations {
		str += fmt.Sprintf("0x%-10X%-30s%s\n", reloc.VirtualAddress, RelocationTypesI386[reloc.Type], reloc.Symbol.Name)
	}
	return str
}
...
func (d ImportData32) String() string {
	return structString(0, "Import MetaData", d)
}
...
func (d ImportData64) String() string {
	return structString(0, "Import MetaData 64bit", d)
}
...
func (e ExportData) String() string {
	return structString(0, "Export MetaData", e)
}
...
type VersionInfoBlock struct {
	...
	ValueLength uint16 // Field length (if any)
	Type        uint16 // Field type (0 = binary, 1 = text)
	// Label      uint8[1] // Field name (block key) (always NULL terminated)

	//////////
	// WORD padding1[]; // Padding, if any (ALIGNMENT)
	// xxxxx Field[]; // Field data, if any (*ALIGNED*)
