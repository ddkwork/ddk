...
func PE(filename string) (pe *PEFile, err error) {
	...
	if mylog.Check(pe.parseInterface(&pe.DosHeader.ImageDosHeader, offset, pe.DosHeader.size)); err != nil {
		return nil, err
	}
	...
	if mylog.Check(pe.parseInterface(&pe.NTHeader.ImageNTHeader, offset, pe.NTHeader.size)); err != nil {
		return nil, err
	}
	...
	if mylog.Check(pe.parseInterface(&pe.FileHeader.ImageFileHeader, offset, pe.FileHeader.size)); err != nil {
		return nil, err
	}
	...
	if mylog.Check(pe.parseInterface(&pe.OptionalHeader.ImageOptionalHeader32, offset, pe.OptionalHeader.size)); err != nil {
		return nil, err
	}
	...
	if pe.OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC {
		...
		if mylog.Check(pe.parseInterface(&pe.OptionalHeader64.ImageOptionalHeader64, offset, pe.OptionalHeader64.size)); err != nil {
			return nil, err
		}
	...
	if pe.FileHeader.PointerToSymbolTable != 0 {
		...
		for symbolOffset, i := int(pe.FileHeader.PointerToSymbolTable), 0; i < numberOfSymbols; i++ {
			...
			if mylog.Check(pe.parseInterface(&pe.SymbolTable[i].ImageSymbol, symbolOffset, pe.SymbolTable[i].size)); err != nil {
				return nil, err
			}
		...
		if mylog.Check(pe.parseInterface(&stringTableSize, pe.StringTableOffset, 4)); err != nil {
			return nil, err
		}
	...
	for i := uint32(0); i < numRvaAndSizes&0x7fffffff; i++ {
		...
		if mylog.Check(pe.parseInterface(&dirEntry.ImageDataDirectory, offset, dirEntry.size)); err != nil {
			return nil, err
		}
...
func (p *PEFile) parseSections(offset int) (newOffset int, err error) {
	...
	for i := uint32(0); i < uint32(p.FileHeader.NumberOfSections); i++ {
		...
		if mylog.Check(p.parseInterface(&section.ImageSectionHeader, newOffset, section.size)); err != nil {
			return 0, err
		}
		...
		if section.PointerToRelocations != uint32(0) {
			...
			for relocOffset, relocIdx := int(section.PointerToRelocations), 0; relocIdx < numberOfRelocs; relocIdx++ {
				...
				if mylog.Check(p.parseInterface(&section.Relocations[relocIdx].ImageRelocation, relocOffset, section.Relocations[relocIdx].size)); err != nil {
					return 0, err
				}
...
// [ Microsoft Portable Executable and Common Client File String Specification ]
