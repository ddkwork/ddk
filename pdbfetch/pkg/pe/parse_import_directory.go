...
func (p *PEFile) parseImportDirectory(rva, size uint32) (err error) {
	...
	for {
		...
		if mylog.Check(p.parseInterface(&importDesc.ImageImportDescriptor, fileOffset, importDesc.size)); err != nil {
			return err
		}
		...
		if p.OptionalHeader64 != nil {
			if mylog.Check(p.parseImports64(importDesc)); err != nil {
				return err
			}
			...
			if mylog.Check(p.parseImports(importDesc)); err != nil {
				return err
			}
			// Give pretty names to well known dll files
	...
	for idx := uint32(0); idx < uint32(len(table)); idx++ {
		...
		if table[idx].AddressOfData > 0 {
			...
			if table[idx].AddressOfData&ordinalFlag > 0 {
				...
				if mylog.Check(p.parseInterface(&imp.Hint, int(imp.HintNameTableRva), 2)); err != nil {
					return err
				}
