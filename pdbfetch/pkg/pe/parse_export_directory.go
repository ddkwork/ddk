...
func (p *PEFile) parseExportDirectory(rva, size uint32) (err error) {
	...
	if mylog.Check(p.parseInterface(&exportDir.ImageExportDirectory, start, exportDir.size)); err != nil {
		return err
	}
	...
	for i := uint32(0); i < numberOfNames; i++ {
		...
		if mylog.Check(p.parseInterface(&symNameAddr, sym.NameOffset, 4)); err != nil {
			return err
		}
		...
		if mylog.Check(p.parseInterface(&sym.Ordinal, sym.OrdinalOffset, 2)); err != nil {
			return err
		}

		// Address
		sym.AddressOffset = startAddrOfFuncs + (int(sym.Ordinal) * 4)
		if mylog.Check(p.parseInterface(&sym.Address, sym.AddressOffset, 4)); err != nil {
			return err
		}
		if sym.Address == 0 {
			continue
